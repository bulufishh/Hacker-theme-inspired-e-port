  // Matrix Background
    ...
    // Boot Sequence
    const bootLines = [
      "Booting Ying Qian's Cybersecurity Portfolio...",
    ...
    function typeLine(line, index = 0) {
      if (index < line.length) {
        ...
        if (currentLine < bootLines.length) {
          setTimeout(() => typeLine(bootLines[currentLine]), 500);
        } else {
    ...
    function showPortfolio() {
      ...
      setTimeout(() => {
        ...
        document.querySelectorAll('section').forEach((section, i) => {
          setTimeout(() => section.classList.add('visible'), i * 300);
        });
      }, 500);
    }
    ...
    window.addEventListener('resize', () => {
      ...
      // Hide mobile nav if screen gets larger
...
function animateBio() {
  ...
  bio.textContent = '';
  bio.style.overflow = 'visible'; // Remove overflow hidden
  bio.style.borderRight = '2px solid lime'; // Add cursor
  ...
  const typing = setInterval(() => {
    if (i < fullText.length) {
      ...
      bio.style.borderRight = 'none'; // Remove cursor
    }
